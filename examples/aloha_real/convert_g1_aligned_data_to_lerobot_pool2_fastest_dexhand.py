...
def load_raw_episode_data(
    ...
    with h5py.File(ep_path_h5, "r") as ep:
        ...
        state_left = ep["/state/left_effector/position"][:]          # (T, 6)
        state_right = ep["/state/right_effector/position"][:]        # (T, 6)

        action_joint = ep["/action/joint/position"][:]               # (T, 14)
        action_left = ep["/action/left_effector/position"][:]        # (T, 6)
        action_right = ep["/action/right_effector/position"][:]      # (T, 6)
...
def create_26d_state_action(joint_positions, left_hand_pos, right_hand_pos, idx, scale=1):
    """创建16维的state和action数据
    
    Args:
        joint_positions: 关节位置数据
        left_hand_pos: 左手位置数据
        right_hand_pos: 右手位置数据
        idx: 时间索引
    
    Returns:
        16维的numpy数组: [左臂关节(7) + 左手(6) + 右臂关节(7) + 右手(6)]
    """
    # 左臂关节 (0-6)
    left_arm_joints = joint_positions[idx, :7]
    # 右臂关节 (7-13)
    right_arm_joints = joint_positions[idx, 7:14]
    # 左手 (0-5)
    left_hand_joints = left_hand_pos[idx]
    # 右手 (0-5)
    right_hand_joints = right_hand_pos[idx]
    
    # print('left_arm_joints:', left_arm_joints, left_arm_joints.shape)
    # print('right_arm_joints:', right_arm_joints, right_arm_joints.shape)
    # print('left_hand_joints:', left_hand_joints, left_hand_joints.shape)
    # print('right_hand_joints:', right_hand_joints, right_hand_joints.shape)

    # 组合成26维数组
    # TODO
    state_26d = np.concatenate((
        left_arm_joints,              # 0-6: 左臂关节
        left_hand_joints,             # 7-12: 左手
        right_arm_joints,             # 13-19: 右臂关节
        right_hand_joints             # 20-25: 右手
    ))

    # print('state_26d:', state_26d, state_26d.shape)
    ...
    for ep_idx in tqdm.tqdm(episodes):
        ...
        def _prepare_one(local_i: int):
            try:
                ...
                current_state = create_26d_state_action(
                    joint_state_data['joint_positions'],
                    # joint_state_data['left_effector_pos'].flatten(),
                    # joint_state_data['right_effector_pos'].flatten(),
                    joint_state_data['left_effector_pos'],
                    joint_state_data['right_effector_pos'],
                    local_i,
                    scale=1,
                )
                next_state = create_26d_state_action(
                    joint_state_data['joint_positions'],
                    # joint_state_data['left_effector_pos'].flatten(),
                    # joint_state_data['right_effector_pos'].flatten(),
                    joint_state_data['left_effector_pos'],
