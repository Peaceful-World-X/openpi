...
class EBenchInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        state_joint = torch.as_tensor(np.array(data["states/joint"], copy=True),dtype=torch.float32)
        state_gripper = torch.as_tensor(np.array(data["states/gripper"], copy=True),dtype=torch.float32,)
        state = torch.cat([state_joint, state_gripper], dim=-1)

        if "actions/joint" in data:
            action_joint = torch.as_tensor(np.array(data["actions/joint"], copy=True),dtype=torch.float32,)
            action_gripper = torch.as_tensor(np.array(data["actions/gripper"], copy=True),dtype=torch.float32,)
            action_base = torch.as_tensor(np.array(data["actions/base"], copy=True),dtype=torch.float32,)
