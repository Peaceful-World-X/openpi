...
class FsqCodebook(nn.Module):
    ...
    def bins_per_dim(self) -> tuple[int]:
        ...
        if self.codebook_type == "fsq":
            ...
            raise ValueError(f"不支持的代码簿类型 {self.codebook_type}。")
    ...
    def _get_bins_fsq(target_codebook_size: int) -> tuple[int]:
        """
        根据原始 FSQ 论文中的代码簿大小获取每维度的箱数（bins）。
        """
        if target_codebook_size == 2**8:
            ...
            raise ValueError(f"不支持的代码簿大小 {target_codebook_size}。")
    ...
    def _get_bins_lfq(target_codebook_size: int) -> tuple[int]:
        """
        根据 Lookup-Free Quantization 论文获取每维度的箱数（每维度 2 个箱）。
        """
        assert target_codebook_size & (target_codebook_size - 1) == 0, "LFQ 的代码簿大小应该是 2 的幂。"
